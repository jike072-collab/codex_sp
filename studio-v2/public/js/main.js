import { el, readProjectPreferences, saveProjectPreferences, showToast, state, workflowMode, workflowModeLabel } from "./core.js";
...
function stageLabelForReset(stage) {
  return { script: "广告脚本", storyboard: "故事板图片", video: "视频结果" }[stage] || stage;
}

async function handleWorkflowModeSwitch(requestedMode) {
  const currentMode = workflowMode(state.project);
  if (requestedMode === currentMode) return;

  try {
    await switchWorkflowMode(requestedMode);
    showToast(`已切换为「${workflowModeLabel(state.project)}」模式。`);
  } catch (error) {
    if (error.code === "WORKFLOW_MODE_RESET_REQUIRED") {
      const stages = (error.resetStages || []).map(stageLabelForReset);
      const stageList = stages.join("、");
      const confirmed = window.confirm(
        `切换到「${workflowModeLabel({ workflowMode: error.requestedWorkflowMode })}」模式需要清除以下内容：${stageList}。\n\n` +
        "上传的商品素材和产品锁定信息会保留。\n\n" +
        "确认切换吗？"
      );
      if (!confirmed) return;
      ...
      try {
        await switchWorkflowMode(requestedMode, true);
        showToast(`已切换为「${workflowModeLabel(state.project)}」模式，请重新生成脚本。`);
      } catch (retryError) {
        showToast(`切换失败：${retryError.message}`);
      }
    } else {
...
function wireEvents() {
  ...
  el("workspace").addEventListener("click", (event) => {
    ...
    if (modeSegment) {
      handleWorkflowModeSwitch(modeSegment.dataset.workflowMode);
      return;
    }
